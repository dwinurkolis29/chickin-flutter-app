...
class _BroilerEncyclopediaScreenState extends State<BroilerEncyclopediaScreen> {
  ...
  // Tab kalkulator cepat (0: FCR, 1: IP)
  int _calculatorTab = 0;
  final TextEditingController _calcPakanCtrl = TextEditingController(text: '1700');
  final TextEditingController _calcBobotCtrl = TextEditingController(text: '1000');
  final TextEditingController _calcLivabilityCtrl = TextEditingController(text: '96');
  final TextEditingController _calcAvgBwCtrl = TextEditingController(text: '1.8');
  final TextEditingController _calcAgeCtrl = TextEditingController(text: '35');
  final TextEditingController _calcFcrCtrl = TextEditingController(text: '1.7');

  double _calcFcrResult = 1.70;
  double _calcIpResult = 289.71;
  ...
  void dispose() {
    ...
    _calcPakanCtrl.dispose();
    _calcBobotCtrl.dispose();
    _calcLivabilityCtrl.dispose();
    _calcAvgBwCtrl.dispose();
    _calcAgeCtrl.dispose();
    _calcFcrCtrl.dispose();
    super.dispose();
  }

  void _recalculateFcr() {
    final pakan = double.tryParse(_calcPakanCtrl.text.replaceAll(',', '.')) ?? 0;
    final bobot = double.tryParse(_calcBobotCtrl.text.replaceAll(',', '.')) ?? 0;
    setState(() {
      _calcFcrResult = (bobot > 0 && pakan > 0) ? pakan / bobot : 0.0;
    });
  }

  void _recalculateIp() {
    final live = double.tryParse(_calcLivabilityCtrl.text.replaceAll(',', '.')) ?? 0;
    final bw = double.tryParse(_calcAvgBwCtrl.text.replaceAll(',', '.')) ?? 0;
    final age = double.tryParse(_calcAgeCtrl.text.replaceAll(',', '.')) ?? 0;
    final fcr = double.tryParse(_calcFcrCtrl.text.replaceAll(',', '.')) ?? 0;

    setState(() {
      if (age > 0 && fcr > 0) {
        _calcIpResult = (live * bw * 100.0) / (age * fcr);
      } else {
        _calcIpResult = 0.0;
      }
    });
          ...
          child: ConstrainedBox(
            ...
            child: CustomScrollView(
              slivers: [
                ...
                // ── 4. Mini Kalkulator Cepat Interaktif ───────────────────────
                SliverToBoxAdapter(
                  child: Padding(
                    padding: const EdgeInsets.fromLTRB(16, 12, 16, 8),
                    child: _buildQuickCalculator(context),
                  ),
                ),

                // ── 5. List Kartu Ensiklopedia ───────────────────────────────
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.calculate_outlined, color: cs.primary, size: 22),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Uji coba masukkan angka kandang Anda di sini',
                        style: tt.bodySmall?.copyWith(
                          color: cs.onSurfaceVariant,
                          fontSize: 11.5,
                        ),
                  ...
                  Expanded(
                    child: InkWell(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: _calculatorTab == 0 ? cs.primary : Colors.transparent,
                          borderRadius: BorderRadius.circular(AppTheme.pillRadius),
                        ),
                        ...
                        child: Text(
                          ...
                          style: tt.labelMedium?.copyWith(
                            color: _calculatorTab == 0 ? cs.onPrimary : cs.onSurfaceVariant,
                            fontWeight: FontWeight.bold,
                          ),
                        ),
                      ),
                  ...
                  Expanded(
                    child: InkWell(
                      ...
                      child: Container(
                        padding: const EdgeInsets.symmetric(vertical: 8),
                        decoration: BoxDecoration(
                          color: _calculatorTab == 1 ? cs.primary : Colors.transparent,
                          borderRadius: BorderRadius.circular(AppTheme.pillRadius),
                        ),
                        alignment: Alignment.center,
                        child: Text(
                          '2. Hitung IP (Performa)',
                          style: tt.labelMedium?.copyWith(
                            color: _calculatorTab == 1 ? cs.onPrimary : cs.onSurfaceVariant,
                            fontWeight: FontWeight.bold,
                          ),
                        ),
                      ),
            ...
            // Form Input Kalkulator FCR
            if (_calculatorTab == 0) ...[
              Row(
                children: [
                  Expanded(
                    child: _buildCalcInputField(
                      controller: _calcPakanCtrl,
                      label: 'Total Pakan (kg)',
                      hint: '1700',
                      onChanged: (_) => _recalculateFcr(),
                    ),
                  ),
                  const SizedBox(width: 10),
                  Expanded(
                    child: _buildCalcInputField(
                      controller: _calcBobotCtrl,
              ...
              const SizedBox(height: 12),
              // Hasil FCR
              _buildCalcResultBanner(
                title: 'Hasil FCR:',
                valueText: _calcFcrResult.toStringAsFixed(2),
                subText: _getFcrStatusText(_calcFcrResult),
                color: _getFcrColor(_calcFcrResult),
                bgColor: _getFcrBgColor(_calcFcrResult),
              ),
            ] else ...[
              // Form Input Kalkulator IP
              Row(
                children: [
                  Expanded(
                    child: _buildCalcInputField(
                      controller: _calcLivabilityCtrl,
                      label: 'Daya Hidup (%)',
                      hint: '96',
                      onChanged: (_) => _recalculateIp(),
                    ),
                  ),
                  const SizedBox(width: 10),
                  Expanded(
                    child: _buildCalcInputField(
                      controller: _calcAvgBwCtrl,
                      label: 'Bobot Ayam (kg)',
                      hint: '1.8',
                      onChanged: (_) => _recalculateIp(),
                    ),
              ...
              // Hasil IP
              _buildCalcResultBanner(
                title: 'Hasil Indeks Prestasi (IP):',
                valueText: _calcIpResult.toStringAsFixed(1),
                subText: _getIpStatusText(_calcIpResult),
                color: _getIpColor(_calcIpResult),
                bgColor: _getIpBgColor(_calcIpResult),
              ),
            ],
          ],
        ),
  ...
  Color _getFcrColor(double fcr) {
    if (fcr <= 0) return AppColors.primary;
    if (fcr <= 1.80) return AppColors.fcrGoodText;
    if (fcr <= 2.20) return AppColors.fcrWarnText;
    return AppColors.fcrBadText;
  }

  Color _getFcrBgColor(double fcr) {
    if (fcr <= 0) return Theme.of(context).colorScheme.surfaceContainer;
    if (fcr <= 1.80) return AppColors.fcrGoodBg;
    if (fcr <= 2.20) return AppColors.fcrWarnBg;
    return AppColors.fcrBadBg;
  }

  String _getFcrStatusText(double fcr) {
    if (fcr <= 0) return 'Masukkan angka valid';
    if (fcr <= 1.80) return 'Efisien (Sangat Bagus)';
    if (fcr <= 2.20) return 'Cukup (Periksa ceceran pakan)';
    return 'Boros (Evaluasi segera)';
  }

  Color _getIpColor(double ip) {
    if (ip <= 0) return AppColors.primary;
    if (ip >= 350) return AppColors.fcrGoodText;
    if (ip >= 300) return AppColors.fcrWarnText;
    return AppColors.fcrBadText;
  }

  Color _getIpBgColor(double ip) {
    if (ip <= 0) return Theme.of(context).colorScheme.surfaceContainer;
    if (ip >= 350) return AppColors.fcrGoodBg;
    if (ip >= 300) return AppColors.fcrWarnBg;
    return AppColors.fcrBadBg;
  }

  String _getIpStatusText(double ip) {
    if (ip <= 0) return 'Masukkan angka valid';
    if (ip >= 400) return 'Istimewa / Luar Biasa';
    if (ip >= 350) return 'Sangat Baik (Untung Besar)';
    if (ip >= 300) return 'Sedang (Performa Standar)';
    return 'Kurang (Perlu Perbaikan)';
  }

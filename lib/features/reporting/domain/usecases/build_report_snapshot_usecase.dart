...
class BuildReportSnapshotUseCase {
  ...
  PeriodReport execute(PeriodData period) {
    ...
    final snapshot = PeriodSnapshot(
      ...
      finalAvgWeightGram:
          s.finalPopulation > 0 && s.finalBiomass > 0
    ...
    final weightGainKg = (s.finalBiomass - initBiomass).clamp(
      0.0,
      double.infinity,
    );

    final computedIP =
        (s.ipScore != null && s.ipScore! > 0)
            ? s.ipScore
            : (snapshot.durationDays > 0 &&
                    s.finalFCR > 0 &&
                    analytics.survivalRate > 0 &&
                    snapshot.finalAvgWeightGram > 0
                ? ((analytics.survivalRate *
                        (snapshot.finalAvgWeightGram / 1000.0) *
                        100.0) /
                    (snapshot.durationDays * s.finalFCR))
                : null);

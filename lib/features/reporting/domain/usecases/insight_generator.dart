...
class InsightGenerator {
  ...
  List<String> execute(PeriodSnapshot snapshot, int initialPopulation) {
    ...
    if (snapshot.harvestedChicks != null &&
        snapshot.harvestedWeightKg != null) {
    ...
    if (snapshot.ipScore != null && snapshot.ipScore! > 0) {
      ...
      if (ip >= 400) {
        insights.add(
          'Indeks Performa Istimewa (IP ${ip.toStringAsFixed(0)}) — efisiensi panen sangat tinggi',
        );
      } else if (ip >= 350) {
        insights.add(
          'Indeks Performa Sangat Baik (IP ${ip.toStringAsFixed(0)}) — performa panen optimal',
        );
      } else if (ip >= 300) {
        insights.add(
          'Indeks Performa Baik (IP ${ip.toStringAsFixed(0)}) — standar broiler tercapai',
        );
      } else {
        insights.add(
          'Indeks Performa Kurang (IP ${ip.toStringAsFixed(0)}) — evaluasi FCR dan durasi panen',
        );
      }
    }
    ...
    if (snapshot.finalFCR > 0) {
      if (snapshot.finalFCR <= _goodFcrMax) {
        insights.add(
          'FCR sangat baik (${snapshot.finalFCR.toStringAsFixed(2)}) — efisiensi pakan optimal',
        );
      } else if (snapshot.finalFCR >= _badFcrMin) {
        insights.add(
          'FCR tinggi (${snapshot.finalFCR.toStringAsFixed(2)}) — evaluasi manajemen pakan',
        );
      }
    }
    ...
    if (mortalityRate <= _lowMortalityMax) {
      insights.add(
        'Mortalitas rendah (${mortalityRate.toStringAsFixed(1)}%) — manajemen kesehatan baik',
      );
    } else if (mortalityRate >= _highMortalityMin) {
      insights.add(
        'Mortalitas tinggi (${mortalityRate.toStringAsFixed(1)}%) — perlu evaluasi biosekuriti',
      );
    }
    ...
    if (snapshot.avgDailyGain > 0) {
      if (snapshot.avgDailyGain >= 50) {
        insights.add(
          'Pertumbuhan harian baik (${snapshot.avgDailyGain.toStringAsFixed(1)} g/hari)',
        );
      } else if (snapshot.avgDailyGain < 30) {
        insights.add(
          'Pertumbuhan harian lambat (${snapshot.avgDailyGain.toStringAsFixed(1)} g/hari) — cek kualitas pakan',
        );
      }
    }

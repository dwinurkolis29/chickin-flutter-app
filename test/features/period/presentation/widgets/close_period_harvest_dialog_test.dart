...
void main() {
  ...
  group('ClosePeriodHarvestDialog Tests', () {
    testWidgets('menampilkan dialog tutup panen dengan form interaktif', (
      tester,
    ) async {
      ...
      expect(
        find.textContaining(
          'Apakah periode "Periode Batch 1" sudah selesai dipanen?',
        ),
        findsOneWidget,
      );
    ...
    testWidgets(
      'memasukkan data panen menghitung live preview rata-rata bobot dan mengembalikan result',
      (tester) async {
        ClosePeriodHarvestResult? result;

        await tester.pumpWidget(
          createTestWidget((context) async {
            result = await ClosePeriodHarvestDialog.show(
              context: context,
              period: testPeriod,
            );
          }),
        ...
        expect(
          find.textContaining('Rata-rata Bobot Panen: 1.80 kg/ekor'),
          findsOneWidget,
        );
